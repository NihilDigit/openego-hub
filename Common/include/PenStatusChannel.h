...
enum class TouchProviderState : uint8_t {
    ...
    SwitchingToHuawei,
    Error,
    // 已废弃，不再发布。挂起期间曾经用它表示「宿主被服务停掉、租约仍在托盘手里」，而那是
    // 一个两个提供方都不在跑、又没有任何看门狗的状态：实测一次息屏就是 68 秒无提供方，
    // Resume 事件一旦丢失便无限期停住。现在挂起走完整的交还，对外发布真实的 Huawei，
    // 「正在挂起」由协调器内部标志表达。值保留在这里不动：它是跨进程 ABI，删掉会让后面的
    // 值全部错位，而两个可执行是分开构建的。
    EGoSuspended,
};
